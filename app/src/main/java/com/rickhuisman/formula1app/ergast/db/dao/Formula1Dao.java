...
public interface Formula1Dao {
    ...
    @Query("SELECT S.*, D.driverId AS driver_driverId, driverRef AS driver_driverRef, number AS driver_number, code AS driver_code, forename AS driver_forename, surname AS driver_surname, dob AS driver_dob, nationality AS driver_nationality, url AS driver_url FROM driverStandings S JOIN drivers D ON D.driverId = S.driverId WHERE raceId = :raceId ORDER BY position")
    LiveData<List<DriverStandingsWithDriver>> getDriverStandingsWithDriver(int raceId);

    @Query("SELECT S.*, C.constructorId AS constructor_constructorId, constructorRef AS constructor_constructorRef, name AS constructor_name, nationality AS constructor_nationality, url AS constructor_url FROM constructorStandings S JOIN constructors C ON C.constructorId = S.constructorId WHERE raceId = :raceId ORDER BY position")
    LiveData<List<ConstructorStandingsWithConstructor>> getConstructorStandingsWithConstructor(int raceId);
}